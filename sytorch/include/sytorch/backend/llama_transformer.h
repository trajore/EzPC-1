...
public:
    ...
    void silu(const Tensor<T> &in, const Tensor<T> &out, u64 scale, u64 mode = 0)
    {
        u64 sz = in.size();
        always_assert(sz == out.size());
        if (mode == 0)
    ...
    void sin(const Tensor<T> &in, const Tensor<T> &out, u64 scale)
    {
        u64 len = in.size();
        for (u64 i = 0; i < len; i++){
            i64 temp = (in.data[i] + (1LL << (bw - 1))) % (1LL << bw) - (1LL << (bw - 1));
            float x = ((float) temp)/std::pow(2,scale);
            i64 res = (std::sin(x))*std::pow(2,scale);
            out.data[i] = (res + (1LL << bw)) % (1LL << bw);
        }
    }

    void cos(const Tensor<T> &in, const Tensor<T> &out, u64 scale)
    {
        u64 len = in.size();
        for (u64 i = 0; i < len; i++){
            i64 temp = (in.data[i] + (1LL << (bw - 1))) % (1LL << bw) - (1LL << (bw - 1));
            float x = ((float) temp)/std::pow(2,scale);
            i64 res = (std::cos(x))*std::pow(2,scale);
            out.data[i] = (res + (1LL << bw)) % (1LL << bw);
        }
    }

    void rotate_half(Tensor<T> &x, Tensor<T> &y)
    {
        always_assert(x.is_same_shape(y));

        u64 channels = x.shape.back();
        u64 len = x.size() / channels;
        for (u64 i = 0; i < len; i++){
            u64 ind = 0;
    ...
    void rotary_embedding_before_sin_cos(Tensor<T> &x, Tensor<T> &y, u64 scale, u64 max_position_embeddings, u64 base)
    {
        always_assert(x.is_same_shape(y));
    ...
    void rmsnorm(const Tensor1D<T> &A, const Tensor1D<T> &B, const Tensor<T> &x, Tensor<T> &y, u64 scale)
    {
        ...
        u64 s2 = x.shape.back();
        u64 s1 = x.size() / s2;
        // SlothRMSNorm(s1, s2, x.data, A.data, y.data, scale);
    ...
    void doOptimizeSilu(LayerGraphNode<T> *node, LayerGraphNode<T> *root)
    {
        if (node->layer->doTruncationForward) 
        {
            if (node->children.size() == 1) 
