...
template <typename T>
void ClearText<T>::gelu(const Tensor<T> &in, const Tensor<T> &out, u64 scale, u64 mode)
{
    ...
    fastfor(in.size(), [&](u64 i) {
        ...
        T r = in.data[i] > 0 ? in.data[i] : 0;
        T rSmall = in.data[0] > 0 ? inpSmall : 0;
...
template <typename T>
void ClearText<T>::silu(const Tensor<T> &in, const Tensor<T> &out, u64 scale, u64 mode)
{
    always_assert(scale == 12);
    fastfor(in.size(), [&](u64 i) {
        auto inpSmall = in.data[i];
    ...
    // fastfor(in.size(), [&](u64 i) {
    //     float x = ((float) in.data[i])/std::pow(2,scale);
    //     out.data[i] = (x / (1 + std::exp(-x)))*std::pow(2,scale);
    // });
}
...
template <typename T>
void ClearText<T>::sin(const Tensor<T> &in, const Tensor<T> &out, u64 scale)
{
    fastfor(in.size(), [&](u64 i) {
        // out.data[i] = (std::sin(in.data[i])*std::pow(2,scale));
        float x = ((float) in.data[i])/std::pow(2,scale);
        out.data[i] = (std::sin(x))*std::pow(2,scale);
    });
}

template <typename T>
void ClearText<T>::cos(const Tensor<T> &in, const Tensor<T> &out, u64 scale)
{
    fastfor(in.size(), [&](u64 i) {
        // out.data[i] = (std::cos(in.data[i])*std::pow(2,scale));
        float x = ((float) in.data[i])/std::pow(2,scale);
        out.data[i] = (std::cos(x))*std::pow(2,scale);
    });
}

template <typename T>
void ClearText<T>::rotate_half(Tensor<T> &x, Tensor<T> &y)
{
    always_assert(x.is_same_shape(y));
...
template <typename T>
void ClearText<T>::rotary_embedding_before_sin_cos(Tensor<T> &x, Tensor<T> &y, u64 scale, u64 max_position_embeddings, u64 base)
{
    always_assert(x.is_same_shape(y));
    ...
    for (u64 i = 0; i < n_seq; ++i) {
        ...
        for (u64 j = 0; j < dim; j+=2) {
            ...
            // y_2d(i, ind) = i * scalar;
            y_2d(i, ind) = (i * scalar)*(std::pow(2,scale));
